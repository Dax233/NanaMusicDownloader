...
'''
version='Version: 2024/06/04 by Dax233'
...
splitword = ' - Cover- ' # 标题分词，用于提取真正的歌名，如此处我用 " - Cover- " 作为分割词
...
def search_parse():
    ...
    try:
        ...
        ## 判定相似度C:\Users\13218\Desktop\python\NNHRDownloader\Lrclog
...
def lrc_make():
    ...
    try:
        if 'nolyric' in lrc_json or '"nolyric":true' in lrc_json:
            ...
            print(done+'检测到纯音乐: '+filename)
        else:
            with open(target_file+'/'+filename+'.lrc', 'w', encoding='utf-8') as f:
                f.write(lrc_json['lyric'])
            print(success+'LRC写入成功: '+filename)
...
def main():
    ...
    for music_file in todo_files:
        ...
        file_name = filename.split(splitword)[0]
        search_error = 0
        search(file_name)
        if search_result == '':
            errorlist.append(music_file)
